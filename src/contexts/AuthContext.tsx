...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  }, [user, setUsers, setActionLoading]);

  const fetchPlans = useCallback(async () => {
    setActionLoading(true);
    try {
      const data = await fetchApi('/plans', { method: 'GET' }, user?.id, user?.role);
      setPlans(data);
    } catch (error: any) {
  ...
  }, [user, setPlans, setActionLoading]);

  const fetchTrainingVideos = useCallback(async (uploadedById?: string) => {
    setActionLoading(true);
    try {
      const endpoint = uploadedById ? `/training-videos?uploadedById=${uploadedById}` : '/training-videos';
      const data = await fetchApi(endpoint, { method: 'GET' }, user?.id, user?.role);
  ...
  }, [user, setTrainingVideos, setActionLoading]);

  const fetchNutritionFiles = useCallback(async (options?: { uploadedById?: string; forTraineeId?: string }) => {
    setActionLoading(true);
    try {
      let endpoint = '/nutrition-files';
      if (options?.uploadedById) endpoint += `?uploadedById=${options.uploadedById}`;
      if (options?.forTraineeId) endpoint += `${options.uploadedById ? '&' : '?'}forTraineeId=${options.forTraineeId}`;
      const data = await fetchApi(endpoint, { method: 'GET' }, user?.id, user?.role);
      setNutritionFiles(data);
    } catch (error: any) {
      console.error('Error fetching nutrition files:', error);
    } finally {
      setActionLoading(false);
    }
  }, [user, setNutritionFiles, setActionLoading]);

  const fetchTraineeSchedule = useCallback(async (traineeId: string) => {
    if (!user) return;
    setActionLoading(true);
    try {
      const data = await fetchApi(`/trainee-schedule/${traineeId}`, { method: 'GET' }, user?.id, user?.role);
      setTraineeSchedule(data);
    } catch (error: any) {
      console.error('Error fetching trainee schedule:', error);
    } finally {
      setActionLoading(false);
    }
  }, [user, setTraineeSchedule, setActionLoading]);

  const fetchMessages = useCallback(async (chatPartnerId: string) => {
    if (!user) return;
    setActionLoading(true);
    try {
      const data = await fetchApi(`/messages/${chatPartnerId}`, { method: 'GET' }, user?.id, user?.role);
  ...
  }, [user, setChatMessages, setActionLoading]);

  const fetchNotifications = useCallback(async () => {
    if (!user) return;
    setActionLoading(true);
    try {
      const data = await fetchApi('/notifications', { method: 'GET' }, user?.id, user?.role);
  ...
  }, [user, setUserNotifications, setActionLoading]);

  const fetchAllRatings = useCallback(async () => {
    if (user?.role !== UserRole.ADMIN) return;
    setActionLoading(true);
    try {
      const data = await fetchApi('/ratings', { method: 'GET' }, user?.id, user?.role);
      setAllRatings(data);
    } catch (error: any) {
  ...
  }, [user, setAllRatings, setActionLoading]);
  ...
  useEffect(() => {
    ...
    attemptAutoLogin();
  }, [fetchInitialDataForUser, clearAllData]); // Added clearAllData dependency
